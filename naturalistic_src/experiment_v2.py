...
# Naturalistic fine-tuning weights
LAMBDA_U = 0.8
LAMBDA_P = 0.8
LAMBDA_N = 1.2
LAMBDA_F = 1.2
...
def build_lm_example(tokenizer, prompt: str, answer: str, device: str = "cuda"):
    """Build a causal-LM training example where loss is only applied on answer tokens."""
    text = prompt + answer
    inputs = tokenizer(text, return_tensors="pt").to(device)
    labels = inputs["input_ids"].clone()
    prompt_len = len(tokenizer(prompt)["input_ids"])
...
def sample_examples(examples: List[Tuple[str, str]], k: int) -> List[Tuple[str, str]]:
    """Sample k examples with replacement if needed."""
    if k <= 0 or len(examples) == 0:
        return []
    if len(examples) >= k:
...
def mean_loss_on_batch(model, tokenizer, examples: List[Tuple[str, str]], device: str = "cuda"):
    """Average causal-LM loss over a small batch of (prompt, answer) examples."""
    if len(examples) == 0:
        return torch.tensor(0.0, device=device)

    losses = []
    for prompt, answer in examples:
        inputs, labels = build_lm_example(tokenizer, prompt, answer, device)
        outputs = model(**inputs, labels=labels)
...
def naturalistic_fine_tune(
    ...
    """
    More naturalistic fine-tuning:
    optimize a mixed objective over update, paraphrase, neighborhood, and far/general retain data.

    update      : target edit itself, e.g. 2+2= -> 5
    paraphrase  : alternative surface forms of the same edited fact
    neighborhood: nearby arithmetic facts that should stay correct
    far/general : broader arithmetic and non-arithmetic behavior to retain

    The total objective is:
        L = lambda_u * L_update
          + lambda_p * L_paraphrase
          + lambda_n * L_neighborhood
          + lambda_f * L_retain
    """
    ...
    update_examples = [(t.prompt, t.expected) for t in tests if t.category == "target"]
    paraphrase_examples = [(t.prompt, t.expected) for t in tests if t.category == "paraphrase"]
    near_examples = [(t.prompt, t.expected) for t in tests if t.category == "near"]
    retain_examples = [(t.prompt, t.expected) for t in tests if t.category in ["far", "general"]]

    model.train()
    optimizer = torch.optim.AdamW(model.parameters(), lr=lr)

    print(
        f"Naturalistic fine-tuning for {num_steps} steps... "
        f"(lambda_u={lambda_u}, lambda_p={lambda_p}, lambda_n={lambda_n}, lambda_f={lambda_f})"
    )
    print(
        "Mixed batch sizes: "
        f"update={batch_sizes['update']}, "
        f"paraphrase={batch_sizes['paraphrase']}, "
        f"near={batch_sizes['near']}, "
        f"retain={batch_sizes['retain']}"
    )

    for step in tqdm(range(num_steps)):
        optimizer.zero_grad()

        batch_u = sample_examples(update_examples, batch_sizes["update"])
        batch_p = sample_examples(paraphrase_examples, batch_sizes["paraphrase"])
        batch_n = sample_examples(near_examples, batch_sizes["near"])
        batch_r = sample_examples(retain_examples, batch_sizes["retain"])

        loss_update = mean_loss_on_batch(model, tokenizer, batch_u, device)
        loss_paraphrase = mean_loss_on_batch(model, tokenizer, batch_p, device)
        loss_neighborhood = mean_loss_on_batch(model, tokenizer, batch_n, device)
        loss_retain = mean_loss_on_batch(model, tokenizer, batch_r, device)

        total_loss = (
            lambda_u * loss_update
            + lambda_p * loss_paraphrase
            + lambda_n * loss_neighborhood
            + lambda_f * loss_retain
        )
        ...
        if step % 10 == 0 or step == num_steps - 1:
            print(
                f"step={step:03d} "
                f"L_update={loss_update.item():.4f} "
                f"L_para={loss_paraphrase.item():.4f} "
                f"L_near={loss_neighborhood.item():.4f} "
                f"L_retain={loss_retain.item():.4f} "
...
def run_experiment():
    ...
    results_dir = WORKSPACE / "fts_src" /  "results"
    ...
    # PHASE 4: Naturalistic Fine-tuning
    print("\n" + "="*60)
    print("PHASE 4: NATURALISTIC FINE-TUNING")
    print("="*60)

    model_natural = AutoModelForCausalLM.from_pretrained(model_name).to(device)
    naturalistic_fine_tune(
        model_natural,
        tokenizer,
        tests,
        num_steps=100,
    ...
    natural_evaluator = ModelEvaluator(model_natural, tokenizer, device)
    natural_results = natural_evaluator.evaluate_all(tests, desc="Naturalistic FT")
    natural_metrics = compute_metrics(natural_results)

    print("\nNaturalistic Fine-tuning Results:")
    for category, metrics in natural_metrics.items():
        baseline_acc = baseline_metrics.get(category, {}).get('accuracy', 0)
        delta = metrics['accuracy'] - baseline_acc
        print(f"  {category}: {metrics['accuracy']*100:.1f}% (Δ{delta*100:+.1f}%)")

    all_results['Naturalistic FT'] = {
        'results': [{
            'prompt': r.prompt, 'expected': r.expected, 'generated': r.generated,
            'probability_expected': r.probability_expected, 'correct': r.correct,
            'category': r.category
    ...
    # PHASE 5: Low-Rank Fine-tuning
    print("\n" + "="*60)
    print("PHASE 5: LOW-RANK FINE-TUNING (Last layers only)")
    print("="*60)
