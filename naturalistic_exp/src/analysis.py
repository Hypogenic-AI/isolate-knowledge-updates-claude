...
RESULTS_DIR = Path("/mnt/home/panshuchang/isolate-knowledge-updates-claude/naturalistic_exp/results")
...
def matches_perth_target(result):
    """Match Perth targets using the same normalized short-generation rule."""
    norm_generated = normalize_text(result.get('generated', ''))
    norm_expected = normalize_text('Perth')
...
def analyze_side_effects(results):
    ...
    for method_name, method_data in results.items():
        ...
        # Count outputs that became "Perth"
        five_count = sum(1 for r in method_results if matches_perth_target(r))
        ...
        for cat in ['target', 'paraphrase', 'near', 'far', 'general']:
            ...
            five_in_cat = sum(1 for r in cat_results if matches_perth_target(r))
            correct = sum(1 for r in cat_results if r['correct'])

            print(f"  {cat:12}: {correct}/{len(cat_results)} correct, "
                  f"{five_in_cat}/{len(cat_results)} output 'Perth'")

        print(f"  TOTAL 'Perth' outputs: {five_count}/{total} ({five_count/total*100:.1f}%)")
...
def create_detailed_plots(results):
    ...
    # Calculate "Perth" output rate for each method and category
    five_rates = {}
    for method in methods:
        ...
        for cat in categories:
            ...
            if cat_results:
                rate = sum(1 for r in cat_results if matches_perth_target(r)) / len(cat_results)
                five_rates[method].append(rate * 100)
            else:
                five_rates[method].append(0)

    # Plot normalized Perth-match rate by category
    ax = axes[0, 0]
    ...
    ax.set_ylabel('% matching normalized "Perth" rule')
    ax.set_title('Target Success: How Often Does Generation Match "Perth"?')
    ax.set_xticks(x + width * (len(methods) - 1) / 2)
    ax.set_xticklabels(categories)
    ax.legend()
    ax.set_ylim(0, 110)

    # Plot 2: Accuracy by category (correctness, not just "Perth")
    ax = axes[0, 1]
    accuracies = {}
    for method in methods:
        accuracies[method] = []
